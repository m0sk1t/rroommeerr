(function() {
	angular.module('RROOMMEERR').controller('MainCtrl', ['$scope', '$http',
		function($scope, $http) {
			...
			$scope.d_names = ['артикул', 'коллекция', 'длина', 'ширина', 'толщина', 'метраж пачки', 'количество досок в пачке', 'количество пачек в паллете', 'замок', 'структура', 'фаска', 'класс', 'плотность плиты hdf'];
			$scope.opt = {
				door: null,
				brand: null,
				floor: null,
